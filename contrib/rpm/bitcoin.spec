...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 10595
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 10596
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 20595
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 20596
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 10595
	%{_sbindir}/semanage port -d -p tcp 10596
	%{_sbindir}/semanage port -d -p tcp 20595
	%{_sbindir}/semanage port -d -p tcp 20596
	for selinuxvariant in %{selinux_variants}; do
