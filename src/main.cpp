...
#if defined(NDEBUG)
# error "Jagaricoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfe820cba27c29748193dee00da4468b57a3e2fc457dd64fa77b50059777b5178");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Jagaricoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jagaricoin Signed Message:\n";
...
{
    // Jagaricoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Jagaricoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Jagaricoin: 2 days
static const int64 nTargetSpacing = 2 * 60; // Jagaricoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jagaricoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jagaricoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6646d240819ece1c5934e5c0883e91cbce5ece0c76475baf8a700f334c3b7b9e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040030d7361afa7fdbbb1f4da678ba49c1304a33affee3b15554b5699019cb07a1e433da23abbb8305332afab518f29ab448533c8a8a479f145c7a6617e385dc09") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519321577;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 566603;

        if (fTestNet)
        {
            block.nTime    = 1519321481;
            block.nNonce   = 19417;
        }
        ...
        assert(block.hashMerkleRoot == uint256("beda29947410c7616c52f91511cb1215f4e7c8de790733688735546b78f0ba38"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x61, 0x63, 0x65, 0x67 }; // Jagaricoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JagaricoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JagaricoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JagaricoinMiner(CWallet *pwallet)
{
    printf("JagaricoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jagaricoin-miner");
    ...
    try { loop {
        ...
        printf("Running JagaricoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JagaricoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JagaricoinMiner, pwallet));
}
