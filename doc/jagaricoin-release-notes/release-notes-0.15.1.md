Jagaricoin Core version 0.13.2 is now available from:

  <https://download.www.jagaricoin.tk/jagaricoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/jagaricoin-project/jagaricoin/issues>
...
updates will be released anymore. Without security updates, using a jagaricoin
...
ECDSA signatures inside Jagaricoin transactions now use validation using
...
Previous versions of Jagaricoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Jagaricoin Core was to raise the default minimum
relay fee.

Jagaricoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Jagaricoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running jagaricoind without having to do any manual configuration.
...
Jagaricoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Jagaricoin Core will relay transactions with
...
entirely in Jagaricoin Core 0.13, and it is at this time undecided whether the
...
Jagaricoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Jagaricoin Core automatically creates a hidden service to listen on, without
manual configuration. Jagaricoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Jagaricoin Core is listening, and
...
Jagaricoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Jagaricoin
Core to use floating fees. By default, Jagaricoin Core will use floating
fees.
...
(default: `0.0002` JAGA/kB).

At all times, Jagaricoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) JAGA.
Furthermore, Jagaricoin Core will never create transactions paying less than
...
`jagaricoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 30595 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 30595 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /jagaricoinrpc>
        ProxyPass http://127.0.0.1:10595/
        ProxyPassReverse http://127.0.0.1:10595/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass jagaricoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< jagaricoinrpc:<password>
    </Location>
...
Restarting jagaricoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in jagaricoin.conf
...
jagaricoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/jagaricoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Jagaricoin Core code base has
...
- `jagaricoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `jagaricoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Jagaricoin scripting system that in combination with
...
Jagaricoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Jagaricoin Core.
...
The two phases are distinct in the Jagaricoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Jagaricoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Jagaricoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Jagaricoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of jagaricoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of jagaricoins being spent is exactly
...
- **Increased security for multisig:** Jagaricoin addresses (both P2PKH addresses
  ...
  believed to be provided by Jagaricoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Jagaricoin's current ledger (called the UTXO set),
  ...
  note that Jagaricoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Jagaricoin users to individually opt-in to almost any change in the Jagaricoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Jagaricoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Jagaricoin
client to Jagaricoin Core 0.13.2 to signal for segwit support. When a
...
Since Jagaricoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `jagaricoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `jagaricoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
